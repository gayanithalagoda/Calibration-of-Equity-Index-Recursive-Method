...
# -*- coding: utf-8 -*-
"""
Created on Fri Sep  3 12:08:58 2021
...
x = pd.read_csv("/Users/gayanithalagoda/Desktop/masters thesis/MASTERS_RESEARCH_CODE/Research_Content/Datasets/Simulated_data/stt1.csv")
p = np.log(x)
z = p.diff()
z = np.array(z[0:905])
z = z[1:]
...
gamma = [0.010, 0.07,0.08,0.09]
nu = [0.05205761, 0.06713045, 0.09713045, 0.0787]
fish_gamma = [0,0,0,0]
fish_nu=[0,0,0,0]
...
lambda1 = np.exp(-(gammaIP)*intrate[1]/(nuIP**2) - 0.5*((gammaIP)/nuIP)**2)
Loglikelihood1= -0.5*np.log(2*math.pi*nuIP**2)- (intrate[0]-gammaIP)**2/(2*nuIP**2)
...
for u in range(0,no_passes):
    ...
    for k in range(0,batch+1):#batch+1
        ...
        for i in range(0,state):
            ...
            Gamma[k,i]=math.exp(-(IR[k+1]*(gamma[i])/nu[i]**2) - ((gamma[i])**2)/(2*nu[i]**2))
        ...
        for r in range(0,state):
            ...
            sigmaJ4[:,r]= sumJ4[:,r]+ Gamma[k,r]*sigmaX[r,inta+k-2]*Pi[0,r]*E[:,0]
    ...
    for i in range(0,state):
        theta[i]= nu[i]/ np.sqrt(np.abs((u+1-u)))
        beta[i]= gamma[i]/np.abs((u+1-u)) + 0.5*theta[i]**2
    for k in range(0,batch):
        ...
        intr[k+1] = gammaIP1 + np.random.normal(loc=0.0, scale=nuIP1) # dimension issues
        Loglikelihood[k] = -(-0.5* np.log(2*math.pi*nuIP1**2)- (intrate[inta+k-1]- gammaIP1)**2/(2*nuIP1**2))# if remove k , it works
    ...
    for r in range(0,state):
        ...
        Pi[3,r]= remainderSJ3[r,inta+batch-2]/remainderSO[r,inta+batch-2]
    ...
    for r in range(0,state):
        gamma[r] =(remainderST1[r,inta+batch-1])/remainderSO[r,inta+batch-2]
    for r in range(0,state):
        nu[r] = math.sqrt(abs(remainderST2[r,inta+batch-1] + (gamma[r]**2) * remainderSO[r,inta+batch-2] - 2*gamma[r]*remainderST1[r,inta+batch-2] )/remainderSO[r,inta+batch-2])
    for r in range(0,state):
        fish_gamma[r] = remainderSO[r,inta+batch-2]/(2*nu[r]**2)
    for r in range(0,state):
        fish_nu[r] = (remainderSO[r,inta+batch-2]/nu[r]**2) + 3*(remainderST2[r,inta+batch-1] + (gamma[r]**2) * remainderSO[r,inta+batch-2] - 2*gamma[r]*remainderST1[r,inta+batch-1])/nu[r]**4
    inta= inta+batch
    inte= inte+batch 
